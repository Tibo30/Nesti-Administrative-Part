...
<script>
    ...
    function allRecipesDelete() {
        ...
        deleteRecipe(idRecipe).then((response) => {
            if (response) {
                if (response.success) {
                    const tbody = document.querySelector("#allRecipesTbody");
                    tbody.innerHTML = ""; // we empty the tbody
                    // then we fill it with the new recipes got from the fetch
                    response['recipes'].forEach(element => {
                        const tr = document.createElement("tr");
                        tr.innerHTML = "<td>" + element.id + "</td><td>" + element.name + "</td><td>" + element.difficulty + "</td><td>" + element.number + "</td><td>" + element.time + "</td><td>" + element.chief + "</td><td>" + element.state + "</td><td>" + element.action + "</td>";
